...
- 文件动作中的复制/移动目标和“打开方式”选择器使用异步 `NSOpenPanel.begin`，不会进入同步模态循环；启动器在选择器出现后立即收起。
- Carbon 热键回调显式创建 `MainActor` 任务，不再依赖 `MainActor.assumeIsolated`。
- 模块搜索异常写入隐私化 Unified Logging，正常无结果仍保持静默，便于开发诊断而不泄露查询正文。
...
- 完整 Xcode 环境下 `swift test -Xswiftc -warnings-as-errors` 已通过 38 项测试；CI 会拒绝 Command Line Tools-only 环境并同时执行严格检查、XCTest、应用构建和签名验证。连续输入与逐字符删除已通过本机可访问性回归。
