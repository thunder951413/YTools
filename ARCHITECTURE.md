...
- 复制/移动目标和“打开方式”选择器使用异步 `NSOpenPanel.begin`；动作触发后启动器立即收起，选择器取消不会重新唤起面板或阻塞主 Actor。
...
Carbon 全局热键回调不假定调用线程的 Actor 隔离，而是显式调度到 `MainActor` 后读取动作表。模块异常写入 Unified Logging，模块 ID 可见但错误详情保持隐私；查询正文从不写入日志。正常的“不匹配”仍以空结果表达。
